...
**Epic 2 (Organization structure) is done**, as of `FUTURE-PLAN.md` Session 6 (2026-08-23): the Department self-relation hierarchy, `Station`/`Unit` reference tables, the real FK on `IncidentUnitResponse.unitIdLinked`, the incident-side picker + quick-add, and the `/admin/settings` Admin-only CRUD screen are all built — see this section's "Department hierarchy and Station/Unit" entry above for the data-model shape, and `FUTURE-PLAN.md`'s Sessions 4-6 "Findings" for build-time decisions (delete-behavior choices, the interim Unit-Response-tab regression between Sessions 4 and 5 and its fix, district-level admin scoping deferred to Sessions 8/9). Epic 3 (access control) is next.

- **Personnel** turned out narrower than originally assumed — see Field-verification discipline #5 above. It's access-control/roles only (`FUTURE.md` Epic 3), not a roster. Named-individual data (rank, certs, fit-test, physicals, apparatus assignment) stays a separate, later epic, deliberately not scoped into this pass.
