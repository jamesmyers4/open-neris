...
- **Review & Approve workflow**, kickback-with-notes, and conditional email/in-app notifications — `FUTURE.md` Epic 4, same shape as previously captured here (solo fast-path vs. multi-role chain), now with the notification mechanism (a `Notification` table, no real-time infra needed) and provider choice noted.
...
- Whether a district-level role ever reviews incidents directly, or is oversight/reporting-only (see `FUTURE.md` Epic 4's open items).
- Email delivery provider for Review & Approve notifications (see `FUTURE.md` Epic 4).
