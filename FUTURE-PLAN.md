...
**Session 7 (Self-serve signup flow) is also done — the schema decision it made (nullable `User.clerkId` + `UserStatus` enum) is what Session 8's admin-invite flow builds on next.** Read Session 7's "Findings" before starting Session 8: same manual-verification gap shape as Session 6 (the three signup branches were verified against real Postgres through the real, unmocked logic, not through three actual Clerk sign-ups — this project's E2E suite has only one real Clerk identity, and it's already onboarded before any spec runs). Skip straight to **Session 8** to resume.
