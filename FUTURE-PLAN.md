...
**Session 7 (Self-serve signup flow) and Session 8 (Admin-invite flow) are both done.** Session 7's schema decision (nullable `User.clerkId` + `UserStatus` enum) is exactly what Session 8 built on, as planned — no second migration needed for the same concern. Both sessions share the same manual-verification gap shape: the underlying logic was verified against real Postgres through the real, unmocked code, not through actual Clerk sign-ups/invitation emails — this project's E2E suite has only one real Clerk identity (already onboarded before any spec runs), and firing a genuine invitation email during an unattended session isn't something either session did without the user's say-so. Read Session 8's "Findings" before starting Session 9, since it also covers the district-admin scoping decision (no new `UserRole` — reuses Session 4's hierarchy) Session 9's own role-scoping work builds on. Skip straight to **Session 9** to resume.
