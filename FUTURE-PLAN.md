...
**Session 4 (Organization schema) is implemented, not yet committed** — schema/migration only, per its own scope. Read its "Findings" subsection before starting Session 5: it flags a real, deliberate interim regression (the live Unit Response tab can no longer save until Session 5 lands the real picker) that makes Session 5 non-optional/urgent rather than a nice-to-have follow-up.
