...
- The `type-gating path` describe block's original tripwire test (`TODO_pending_sections_2_7_completeness_gate`, asserting that `getSubmitCompleteness` tagged every check `'core'` and so didn't yet differ by incident type) has been **superseded, not just fixed**, across `FUTURE-PLAN.md` Sessions 1-2: Fire/Medical/HazSit were wired to their real `neris_core`-verified required-schemas first, then Session 2 added Exposures/Unit Response/Rescues-Ff/Rescues-NonFf required-schemas (all `neris_core`-verified against the vendor CSVs, not the tripwire's old always-`'core'` placeholder). The two tests now in that describe block (`'a FIRE-primary incident is blocked on fire-module completeness...'`, `'a MEDICAL-primary incident is blocked until at least one patient record exists'`) assert the real, type-differing behavior directly, and `runCoreOnlyJourney`'s helper now also completes a unit-response step, since `dispatch_unit_response` (`neris_core_app=TRUE`, unconditional per Session 1's finding) is checked regardless of incident type.
