...
function SubmitButton() {
  ...
  return (
    <button type="submit" disabled={pending} className="rounded bg-slate-900 px-4 py-2 text-white disabled:opacity-50">
      {pending ? "Saving…" : "Add firefighter casualty"}
    </button>
  );
}

export function RescueFfForm({ incidentId }: { incidentId: string }) {
  const initialState: CreateRescueState = {};
  const action = createRescueFf.bind(null, incidentId);
  const [state, formAction] = useActionState(action, initialState);

  const [birthMonthYear, setBirthMonthYear] = useState("");
  const [gender, setGender] = useState("");
  const [race, setRace] = useState("");
  const [casualtyRank, setCasualtyRank] = useState("");
  const [casualtyService, setCasualtyService] = useState("");
  ...
  const [mayday, setMayday] = useState("");
  const [maydayRelativeTime, setMaydayRelativeTime] = useState("");
  const [ritActivated, setRitActivated] = useState("");
  ...
  const [casualtyClassification, setCasualtyClassification] = useState("");
  ...
  const [casualtyAction, setCasualtyAction] = useState("");
  const [casualtyPpe, setCasualtyPpe] = useState<string[]>([]);
  ...
  return (
    <form action={formAction} className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <label className="flex flex-col gap-1 text-sm">
            ...
            <input
              ...
              maxLength={255}
              value={casualtyRank}
              onChange={(e) => setCasualtyRank(e.target.value)}
              className="rounded border border-slate-300 px-2 py-1"
            />
          </label>
          <label className="flex flex-col gap-1 text-sm">
            ...
            <input
              ...
              value={casualtyService}
              onChange={(e) => setCasualtyService(e.target.value)}
              className="rounded border border-slate-300 px-2 py-1"
            />
          </label>
          <label className="flex flex-col gap-1 text-sm">
          ...
          <label className="flex flex-col gap-1 text-sm">
            Duty type
            <select name="dutyType" value={dutyType} onChange={(e) => setDutyType(e.target.value)} className="rounded border border-slate-300 px-2 py-1">
              <option value="">—</option>
              {TypeDuty.map((v) => (
                <option key={v} value={v}>
                  {v}
      ...
      <section className="space-y-3">
        <h3 className="text-sm font-medium">Mayday</h3>
        <div className="grid grid-cols-2 gap-3">
          <label className="flex flex-col gap-1 text-sm">
            Mayday called
          ...
          <label className="flex flex-col gap-1 text-sm">
            Mayday relative time
            <select
              name="maydayRelativeTime"
              value={maydayRelativeTime}
              onChange={(e) => setMaydayRelativeTime(e.target.value)}
              className="rounded border border-slate-300 px-2 py-1"
            >
              <option value="">—</option>
          ...
          <label className="flex flex-col gap-1 text-sm">
            ...
            <select
              name="ritActivated"
              value={ritActivated}
              onChange={(e) => setRitActivated(e.target.value)}
              className="rounded border border-slate-300 px-2 py-1"
            >
      ...
      <section className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <label className="flex flex-col gap-1 text-sm">
            Job classification
            <select
              name="casualtyClassification"
              value={casualtyClassification}
              onChange={(e) => setCasualtyClassification(e.target.value)}
              className="rounded border border-slate-300 px-2 py-1"
            >
              <option value="">—</option>
              {TypeJobClassification.map((v) => (
                <option key={v} value={v}>
                  {v}
                </option>
              ))}
          ...
          <label className="flex flex-col gap-1 text-sm">
            Action at time of injury
            <select
              name="casualtyAction"
              value={casualtyAction}
              onChange={(e) => setCasualtyAction(e.target.value)}
              className="rounded border border-slate-300 px-2 py-1"
            >
              <option value="">—</option>
              {TypeCasualtyAction.map((v) => (
                <option key={v} value={v}>
                  {v}
                </option>
              ))}
          ...
          <label className="flex flex-col gap-1 text-sm">
            ...
            <select
              ...
              onChange={(e) => setIncidentCommand(e.target.value)}
              className="rounded border border-slate-300 px-2 py-1"
            >
              <option value="">Unknown</option>
              <option value="true">Yes</option>
          ...
          <label className="flex flex-col gap-1 text-sm">
            Incident timeline
            <select
              name="casualtyTimeline"
              value={casualtyTimeline}
              onChange={(e) => setCasualtyTimeline(e.target.value)}
              className="rounded border border-slate-300 px-2 py-1"
            >
              <option value="">—</option>
              {TypeCasualtyTimeline.map((v) => (
                <option key={v} value={v}>
                  {v}
                </option>
              ))}
        ...
        <CheckboxGroup legend="PPE worn" name="casualtyPpe" options={TypeCasualtyPpe} values={casualtyPpe} onChange={setCasualtyPpe} />
      </section>
